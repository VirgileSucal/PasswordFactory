...
def progressPercent(totalNames, start, names, p, samplesGenerated):
	...
	sys.stdout.write('[%s] %s%s names founded among %d samples generated (%d of %d names) on %s (goal = %.1f%% = %d names)\r' % (bar, percents, '%', samplesGenerated, names, totalNames, timeSinceStart(start), p, nNames))
	sys.stdout.flush()
...
def progress(total, acc, start, epoch, l):
	...
	sys.stdout.write('[%s] %s%s epoch: %d acc: %.3f %% and testing size = %d names => coverage of %.3f %% on %s \r' % (bar, percents, '%', epoch, (100 * acc / epoch), l, (100 * acc / l), timeSinceStart(start)))
	sys.stdout.flush()
...
def testing(decoder, nb_samples, lineTest, percent):
	...
	if nb_samples > 0 :
		...
		print(percent+ ' % of all names (', len(lineTest),') reached in ', i, 'iterations (', timeSinceStart(start), ' s)...')
...
if __name__ == '__main__':
	...
	#
	parser.add_argument('--n', default=10000, type=int, help="number of samples to generate [< 1000]. If < 0, the algorithm will provide names till it reaches the percent (see --p option)")
	...
	decoder = RNNLight(n_letters, 128, n_letters).to(device) #RNN(n_characters, args.hidden_size, n_characters, args.num_layers).to(device)
